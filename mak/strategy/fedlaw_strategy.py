...
from mak.utils.general import set_params, filter_straggler_results
...
class FedLaw(fl.server.strategy.FedAvg):
    ...
    def _get_valid_set(self):
        client_dataset_splits = self.test_data.train_test_split(test_size=self.config_sim['fedlaw_config']['server_valid_ratio'])
        valset = client_dataset_splits["test"]
        # # Now we apply the transform to each batch.
        valset = valset.with_transform(self.apply_transforms)
        val_loader = DataLoader(valset, batch_size=self.config_sim['client']['batch_size'], shuffle=True)
        return val_loader
    ...
    def aggregate_fit(
        ...
        for _, fit_res in results:
            ...
            c_models.append(copy.deepcopy(c_model))  # Append the fully prepared model to the list
    ...
    def _fedlaw_optimization(self, size_weights, parameters, central_node, device):
        ...
        '''
        if self.config_sim['common']['dataset'] == 'cifar10':
            server_lr = 0.01
        else:
            server_lr = 0.005

        cohort_size = len(parameters)
        server_epochs = self.config_sim['fedlaw_config']['server_epochs']
        ...
        if self.config_sim['fedlaw_config']['server_optimizer'] == 'adam':
            optimizer = optim.Adam(optimizee_list, lr=server_lr, betas=(0.5, 0.999))
        elif self.config_sim['fedlaw_config']['server_optimizer'] == 'sgd':
